...
namespace RLTK.FieldOfView.BeveledCorners
{
    ...
    public static class FOV
    {
        ...
        static void Compute<T>(int octant, int2 origin, int range, int x, Slope top, Slope bottom, T map) where T : IVisibilityMap
        {
            for (; x <= (int)range; x++)
            {
                int2 yCoords = ComputeYCoordinates(octant, origin, x, map, ref top, ref bottom);
                int topY = yCoords.x;
                int bottomY = yCoords.y;

                if (!ComputeVisibility(topY, bottomY, range, octant, origin, x, map, ref top, ref bottom))
                    break;
            }
        }
                            ...
                            if (wasOpaque == 0)
                            {
                                ...
                                if (top.Greater(ny, nx))
                                {
                                    if (y == bottomY)
                                    {
                                        ...
                                        Compute(octant, origin, range, x + 1, top, new Slope(ny, nx), map);
                                }
                                else
                                {
                                    if (y == bottomY)
        ...
        struct Slope // represents the slope Y/X as a rational number
        {
            ...
            public bool Greater(int y, int x) { return this.y * x > this.x * y; } // this > y/x
            public bool GreaterOrEqual(int y, int x) { return this.y * x >= this.x * y; } // this >= y/x
            public bool Less(int y, int x) { return this.y * x < this.x * y; } // this < y/x
